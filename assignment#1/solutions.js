...
function toCamelCase(name, uppercase) {
  let words = name.split('_').filter(Boolean); // Remove empty strings from the array

  for (let i = 1; i < words.length; i++) {
    words[i] = words[i][0].toUpperCase() + words[i].slice(1);
  }

  let camelCaseName = words.join('');

  if (uppercase) {
    camelCaseName = camelCaseName[0].toUpperCase() + camelCaseName.slice(1);
  }
...
function createMetaTag(name, content) {
  ...
  let nameAttribute = name.includes(' ') || name.includes('\t') ? `name="${name}"` : `name=${name}`;
  let contentAttribute =
    content.includes(' ') || content.includes('\t') ? `content="${content}"` : `content=${content}`;

  return `<meta ${nameAttribute} ${contentAttribute}>`;
}
...
function parseDateString(value) {
  ...
  // Regular expression to match valid date formats
...
function normalizeDuration(value) {
  ...
  let isValid = false; // Variable to track if the value is valid
  let formattedDuration = null; // Variable to store the formatted duration string

  switch (true) {
    case value.includes(':'): {
      const parts = value.split(':');
      if (parts.length !== 3) {
        break; // Invalid value, exit the switch statement
      }
    ...
    case value.includes('h') && value.includes('m') && value.includes('s'): {
  ...
  switch (true) {
    case !isValid:
    case isNaN(hours):
    case hours < 0 || hours >= 25:
    case minutes < 0 || minutes >= 60:
    case seconds < 0 || seconds >= 60:
      break; // Invalid duration, exit the switch statement
  ...
  return formattedDuration; // Return the formatted duration or null
}
...
function formatDurations(...values) {
  const validDurations = values.map(normalizeDuration).filter((duration) => duration !== null);

  return validDurations.length === 0 ? '()' : `(${validDurations.join(', ')})`;
}
...
function generateLicenseBadge(licenseCode) {
  ...
  if (licenseCode === 'CC-BY') {
    ...
    licenseBadge = '<a href="https://choosealicense.com/no-permission/">All Rights Reserved</a>';
  }
...
function countFalse(...values) {
  ...
  for (const value of values) {
    if (
      ...
      value === 'Faux' ||
      value === '0' ||
      (typeof value === 'number' && value <= 0) // Updated condition
    ) {
      falseCount++;
    }
  }
