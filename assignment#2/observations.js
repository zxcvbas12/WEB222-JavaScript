...
function observationSummary(data) {
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    const date = observed_on_details.date.split('T')[0];
    console.log(`ID-${id} "${species_guess}" (Date: ${date})`);
  }
}
...
function observationSummary2(data) {
  for (let i = 0; i < data.results.length; i++) {
    ...
    const date = observation.observed_on_details.date.split('T')[0];
    console.log(`ID-${id} "${species_guess}" (Date: ${date})`);
  }
}
...
function observationsByGeoPrivacy(data, geoPrivacy) {
  const validValues = ['open', 'obscured', 'private', null];
  if (geoPrivacy !== null && !validValues.includes(geoPrivacy.toLowerCase())) {
 ...
 * }
 ******************************************************************************/
function transformObservation(observation) {
  return {
    id: observation.id,
    speciesGuess: observation.species_guess,
    isResearchQuality: observation.quality_grade === 'research',
    coords: observation.location.split(',').map(Number).reverse(),
    user: observation.user ? `@${observation.user.login}` : '',
    photos: observation.photos.map((photo) => photo.url),
    photosCount: observation.photos.length
  };
}
...
function transformObservations(data) {
  const results = data.results.map(transformObservation);
  return results;
}
/*******************************************************************************
...
function transformObservations2(data) {
  ...
  for (let i = 0; i < data.results.length; i++) {
    ...
    for (let j = 0; j < observation.photos.length; j++) {
      transformedObservation.photos.push(observation.photos[j].url);
    }

    transformedResults.push(transformedObservation);
  }
...
function getObservationsByPositionalAccuracy(data, options = {}) {
  ...
  for (let i = 0; i < data.results.length; i++) {
    ...
    if (eq !== undefined) {
      isValid = observation.positional_accuracy === eq;
    } else if (gt !== undefined && lt !== undefined) {
      isValid = observation.positional_accuracy >= gt && observation.positional_accuracy <= lt;
    } else if (gt !== undefined) {
      isValid = observation.positional_accuracy >= gt;
    } else if (lt !== undefined) {
      isValid = observation.positional_accuracy <= lt;
    } else {
      isValid = true;
    }
...
function getUserStats(data) {
  const results = data.results; // Array of observation objects
  ...
  for (const result of results) {
    ...
    totals.observations += user.observations_count || 0;
    totals.journals += user.journal_posts_count || 0;
    totals.species += user.species_count || 0;
  }

  const averages = {
    observations: totals.observations / count,
    journals: totals.journals / count,
